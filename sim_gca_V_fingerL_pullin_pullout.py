...
def plot_data(fig, pullin_V, pullin_avg, pullin_std, release_V, release_avg, release_std, labels):
    nx, ny = 3, 3
    for idx in range(nx):
        for idy in range(ny):
            i = nx*idy+idx
            ...
            # plt.errorbar(release_V[i], release_avg[i], release_std[i], fmt='r.', capsize=5)
...
if __name__ == "__main__":
    ...
    fig = setup_plot(3, 3, x_label="Voltage (V)", y_label="Time (us)")
    ...
    for i in range(1, len(fingerL_values)+1):
        ...
        labels.append("L={}um".format(fingerL_values[i-1]*1e6))

    plot_data(fig, pullin_V, pullin_avg, pullin_std, release_V, release_avg, release_std, labels)
